...
from api.core.file_io import get_csv_row_count_in_mem, get_file_size_in_mb_in_mem
from api.crud.prospects_files import ProspectsFilesCrud
from api.schemas.prospect_files import ProspectsFile, ProspectsFileCreateResponse, ProspectsFileImportStatusResponse
...
async def create_prospects_file(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Unable to read file"
        )
    is_valid_file_mb_size = file_mb_size <= MAX_FILE_SIZE_MB

    if not is_valid_file_mb_size:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Maximum File Size of 200 MB Reached"
        )
    ...
    if not is_valid_file_rows:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Maximum NUMBER OF 1 MILLION ROWS EXCEEED"
        )
...
@router.post("/prospects-files/{prospects_file_id}/prospects", response_model=schemas.ProspectsFile)
def persist_prospects_files(
    ...
    return ProspectsFile(
        ...
        fileAddress=db_prospects_file.fileAddress,
        preview=db_prospects_file.preview
    )
...
def progress_status(
    ...
    return ProspectsFileImportStatusResponse(
        total=db_prospects_file.total_data_rows,
        done=db_prospects_file.done
    )
